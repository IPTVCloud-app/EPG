...
#   Workers write token lines to stdout; the aggregator reads them
#   via process substitution — no temp files, no flock needed.
# ════════════════════════════════════════════════════════════════
...
classify_log() {
  local log="$1"
  local n_geo n_rate n_5xx n_404 n_ok n_timeout
  ...
  n_timeout=$(grep -ciE "(timeout|ETIMEDOUT|ECONNRESET|ECONNREFUSED|socket hang up|read ECONNRESET|connect ETIMEDOUT)"               "$log" 2>/dev/null || true); n_timeout=${n_timeout:-0}
  printf "[CLASSIFY] geo=%d rate=%d 5xx=%d 404=%d ok=%d timeout=%d\n" \
    "$n_geo" "$n_rate" "$n_5xx" "$n_404" "$n_ok" "$n_timeout" >> "$log"
  if   (( n_geo     > 0 )); then echo "GEO"
  elif (( n_timeout > 0 )); then echo "TIMEOUT"
...
run_batch() {
  ...
  while true; do
    ...
    for s in "${pending[@]}"; do
      ...
      # Geo-block and timeouts are permanent — skip immediately, never retry
      if [[ "$reason" == "GEO" ]]; then
        eval "t_done_${safe}=1"
      ...
      if [[ "$reason" == "TIMEOUT" ]]; then
        eval "t_done_${safe}=1"
        echo "SKIP|${s}|TIMEOUT|${elapsed_fmt}|${attempt}|$(get_site_host "$s")"
        continue
      fi
      ...
      if [[ -s "$output_file" ]]; then
        ...
        # Skip files that contain no usable programme entries
        if (( progs == 0 )); then
          echo "SKIP|${s}|EMPTY|${elapsed_fmt}|${attempt}|${host}"
          continue
        fi
        local partial_flag=""
...
while IFS='|' read -r TOKEN F1 F2 F3 F4 F5 F6 F7 F8; do
  case "$TOKEN" in
    ...
    SKIP)
      _flush_url_stats
      FAIL=$(( FAIL + 1 ))
      local skip_reason="${F2:-UNKNOWN}"
      local skip_label
      case "$skip_reason" in
        GEO)     FAIL_GEO=$(( FAIL_GEO + 1 ));   skip_label="${MAGENTA}403 geo-blocked${NC}" ;;
        TIMEOUT) FAIL_OTHER=$(( FAIL_OTHER + 1 )); skip_label="${YELLOW}timeout${NC}" ;;
        EMPTY)   FAIL_OTHER=$(( FAIL_OTHER + 1 )); skip_label="${DIM}empty (0 programmes)${NC}" ;;
        *)       FAIL_OTHER=$(( FAIL_OTHER + 1 )); skip_label="${DIM}${skip_reason}${NC}" ;;
      esac
      SKIP_LIST+=("$F1 [${skip_reason}, ${F6:-unknown}]")
      printf "${MAGENTA}[SKIP %d/%d]${NC}  %-40s  %b  %s  %s attempt(s)\n" \
        "$(( PASS + FAIL ))" "$TOTAL" "$F1" "$skip_label" "$F4" "$F5"
      ;;
...
echo ""
log "Splitting XML files..."
python3 "$XML_SPLITTER_SCRIPT" "$OUTPUT_DIR"
echo ""
log "Formatting XML files..."
...
if (( ${#SKIP_LIST[@]} > 0 )); then
  echo ""; echo -e "${MAGENTA}Skipped sites:${NC}"
  printf '  • %s\n' "${SKIP_LIST[@]}"
fi